...
#ifndef _LINEAR_output_buf_V_H_
#define _LINEAR_output_buf_V_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "LINEAR_output_buf_V_memcore.h"

SC_MODULE(LINEAR_output_buf_V) {
...
protected:
    ...
    // buffer instances
    LINEAR_output_buf_V_memcore* buffer[BufferCount];
...
public:
    ~LINEAR_output_buf_V();
    SC_CTOR(LINEAR_output_buf_V) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "LINEAR_output_buf_V_memcore_U_";
            name += szTmp;

            buffer[i] = new LINEAR_output_buf_V_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "LINEAR_output_buf_V.trace");
            // ports
